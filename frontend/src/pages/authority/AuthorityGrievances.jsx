...
import React, { useEffect, useState } from "react";
import { supabase } from "../../lib/supbase";
// 🔥 Import the singleton service you just fixed
...
const AuthorityGrievances = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        if (authError || !user) {
          setError("User not logged in or authentication failed.");
          setLoading(false);
          return;
        }
        ...
        // 2. Use AuthorityApi service (Fixes the ReferenceError)
        ...
        if (!dept_id) {
          setError("Department configuration not found.");
          setLoading(false);
          return;
        }
        ...
        // 3. Fetch grievances using the Service (Fixes the ReferenceError)
  ...
  if (loading) return <div className="p-10 text-center text-gray-500">Loading grievances...</div>;
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="space-y-4">
        {grievances.length > 0 ? (
          ...
          <div className="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
            <p className="text-gray-500">No grievances reported for this department yet.</p>
          </div>
        )}
      </div>
    </div>
